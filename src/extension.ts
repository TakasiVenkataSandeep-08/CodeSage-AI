...
import {
  ...
  explainCode,
  refactorCode,
  performOperationOnCodeSelection,
  createProjects,
  copyToClipboard,
  replaceInEditor,
  appendText,
...
export function activate(context: vscode.ExtensionContext) {
  ...
  const refactorCodeSelection = vscode.commands.registerCommand(
    "codesenseai.refactorCode",
    () => {
      refactorCode();
    }
  );
  const explainCodeSelection = vscode.commands.registerCommand(
    "codesenseai.explainCode",
    () => {
      explainCode();
    }
  );
  const askQuestion = vscode.commands.registerCommand(
    "codesenseai.askQuestion",
    () => {
      askQuestions();
    }
  );
  const performCustomOperationOnCodeSelection = vscode.commands.registerCommand(
    "codesenseai.performOperationOnCode",
    () => {
      performOperationOnCodeSelection();
    }
  );
  const createProjectsByDescription = vscode.commands.registerCommand(
    "codesenseai.createProjects",
    () => {
      createProjects();
    }
  );
  ...
  context.subscriptions.push(
    openChat,
    refactorCodeSelection,
    explainCodeSelection,
    performCustomOperationOnCodeSelection,
    askQuestion,
    createProjectsByDescription
  );
}

class CodesenseaiViewProvider implements vscode.WebviewViewProvider {
  ...
  public resolveWebviewView(
    ...
    webviewView.webview.onDidReceiveMessage(async (data) => {
      switch (data.type) {
        ...
        case "log": {
          console.log("log from script", data.value);
          break;
        }
        default:
          break;
  ...
  private _getHtmlForWebview(webview: vscode.Webview) {
    ...
    return `<!DOCTYPE html>
      ...
      <div id="apiKeyViewWrapper">
        <span class="apiKeyTitle">API key will be used to communicate with openAI</span>
        ...
        <span class="chatPromptTitle">Custom Prompts</span>
        <div id="chatPromptWrapper">
        </div>
